...
All 37 static `PaneCommand`s and dynamic `FocusPane { index }` actions route through the view's shared `PaneCommandExecutionOptions`. Split controls and commands work without host configuration: Mullion mints a collision-free internal `PaneId` and clones the focused pane's `D`. Activity-to-pane docking follows the same rule using the destination pane's data. Configure durable/custom ids or initialization with `with_split_factory[_fn]`, `set_split_factory`, `with_new_pane_factory`, and `set_new_pane_factory`; host factories override the defaults and may refuse by returning `None`. Configure resize increments with `with_resize_step` or `set_resize_step`; the legacy public `execute(command, factory, cx)` remains available.
...
Generic command registration, deterministic search, keyboard state, shortcuts, aggregate styles, providers, and modal rendering live in [`gpui-command-palette`](https://github.com/ignition-is-go/gpui-command-palette). Applications with a global palette create and render one `Entity<CommandPaletteState<()>>` at the window/root overlay level, then call `attach_command_palette(&view, palette.clone(), cx)`. Attachment is injection-only: Mullion retains RAII registrations, routes commands through direct weak-view handlers, and resynchronizes all pane-management commands plus activities and host commands for only the focused pane. `with_focused_pane_commands` supplies application-specific focused-pane commands; `refresh_command_palette` re-evaluates provider state changed outside `PaneData`.

For Mullion-only applications, `install_command_palette_for_view(&view, window, cx)` remains the obvious convenience: it creates the same `CommandPaletteState<()>`, attaches the same registration logic, renders it inside Mullion, and installs its window action. `command_palette_for_view` creates the attached but uninstalled convenience palette. Shared applications should prefer root rendering so the palette is never clipped or bounded by Mullion. `CommandPaletteState` owns focus, navigation, and query state explicitly; hosts may subscribe to `CommandPaletteEvent::Executed` for observation, while Mullion commands execute through their registered weak-view handlers after dispatch unwinds.
